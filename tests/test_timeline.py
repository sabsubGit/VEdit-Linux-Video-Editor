...
def linked(timeline):
    ...
    for index, start in enumerate((0, 100)):
        ...
        for kind in ("video", "audio"):
            ...
            timeline.lane_for(kind).insert(clip)
    return timeline
...
class TestTimeline:
    def test_default_has_several_empty_lanes(self, timeline):
        """Empty lanes ship by default so there is somewhere to drop a cutaway
        or a music bed without hunting for an add-track command first."""
        assert [t.name for t in timeline.tracks] == ["V1", "V2", "V3", "A1", "A2", "A3"]
        assert len(timeline.video_tracks) == 3
        assert len(timeline.audio_tracks) == 3

    def test_video_lanes_are_grouped_above_audio(self, timeline):
        kinds = [t.kind for t in timeline.tracks]
        assert kinds == sorted(kinds, key=lambda k: 0 if k == "video" else 1)

    def test_track_count_is_configurable(self):
        timeline = Timeline.default(TimeBase(30), video_tracks=1, audio_tracks=2)
        assert [t.name for t in timeline.tracks] == ["V1", "A1", "A2"]

    def test_duration_is_the_longest_track(self, timeline):
        timeline.video_tracks[0].insert(make_clip(0, 100))
        timeline.audio_tracks[0].insert(make_clip(0, 250, kind="audio"))
        assert timeline.duration == 250

    def test_linked_group_spans_tracks(self, linked):
        video = linked.video_tracks[0].clips[0]
    ...
    def test_snapshot_restore_is_a_deep_copy(self, linked):
        ...
        linked.video_tracks[0].clips[0].tl_start = 999
        linked.restore(snapshot)
        assert linked.video_tracks[0].clips[0].tl_start == 0
...
class TestRazor:
    def test_splits_across_all_tracks(self, linked):
        ...
        assert [c.tl_start for c in linked.video_tracks[0].clips] == [0, 50, 100]

    def test_halves_are_contiguous_and_preserve_source(self, linked):
        ops.razor(linked, 50)
        left, right = linked.video_tracks[0].clips[0], linked.video_tracks[0].clips[1]
    ...
    def test_cut_on_a_boundary_is_a_noop(self, linked):
        before = len(linked.video_tracks[0].clips)
        assert ops.razor(linked, 100) == []
        assert len(linked.video_tracks[0].clips) == before

    def test_cut_in_empty_space_is_a_noop(self, timeline):
        timeline.video_tracks[0].insert(make_clip(0, 10))
    ...
    def test_moving_one_half_leaves_the_other(self, linked):
        ops.razor(linked, 50)
        left = linked.video_tracks[0].clips[0]
        ops.move_clips(linked, [left], 300)
        assert linked.video_tracks[0].clips[0].tl_start == 50, "the right half stayed put"

    def test_locked_track_is_not_cut(self, linked):
        linked.audio_tracks[0].locked = True
        ops.razor(linked, 50)
        assert len(linked.video_tracks[0].clips) == 3
        assert len(linked.audio_tracks[0].clips) == 2, "locked audio lane untouched"
...
class TestDeletion:
    def test_lift_leaves_a_gap(self, linked):
        ops.lift(linked, [linked.video_tracks[0].clips[0]])
        assert [c.tl_start for c in linked.video_tracks[0].clips] == [100]
        assert linked.video_tracks[0].gaps() == [(0, 100)]

    def test_lift_removes_the_linked_partner(self, linked):
        ops.lift(linked, [linked.video_tracks[0].clips[0]])
        assert len(linked.audio_tracks[0].clips) == 1, "audio partner went with it"

    def test_ripple_delete_closes_the_gap(self, linked):
        ops.ripple_delete(linked, [linked.video_tracks[0].clips[0]])
        assert [c.tl_start for c in linked.video_tracks[0].clips] == [0]
        assert linked.video_tracks[0].gaps() == []
    ...
    def test_ripple_delete_shifts_every_track(self, linked):
        ...
        ops.ripple_delete(linked, [linked.video_tracks[0].clips[0]])
        assert linked.video_tracks[0].clips[0].tl_start == 0
        assert linked.audio_tracks[0].clips[0].tl_start == 0
    ...
    def test_ripple_delete_closes_only_its_own_span(self, timeline):
        ...
        ops.trim(timeline, timeline.video_tracks[0].clips[0], "out", 30)
        assert timeline.video_tracks[0].gaps() == [(30, 60), (120, 160)]

        ops.ripple_delete(timeline, [timeline.video_tracks[0].clips[0]])

        # The deleted 0..30 is closed; both other gaps just move back by 30.
        assert [(c.tl_start, c.tl_end) for c in timeline.video_tracks[0].clips] == [(30, 90), (130, 190)]
        assert timeline.video_tracks[0].gaps() == [(0, 30), (90, 130)]
        # And audio tracked video exactly through all of it.
        assert [(c.tl_start, c.tl_end) for c in timeline.audio_tracks[0].clips] == [
            (c.tl_start, c.tl_end) for c in timeline.video_tracks[0].clips
        ]

    def test_ripple_delete_range(self, linked):
        ops.ripple_delete_range(linked, 50, 150)
        # 0-50 survives, 150-200 slides back to 50.
        assert [(c.tl_start, c.tl_end) for c in linked.video_tracks[0].clips] == [(0, 50), (50, 100)]
        assert linked.duration == 100

    def test_delete_on_locked_track_refused(self, linked):
        linked.video_tracks[0].locked = True
...
class TestMove:
    def test_move_shifts_the_whole_link_group(self, timeline):
        for kind in ("video", "audio"):
            timeline.lane_for(kind).insert(make_clip(0, 50, src_length=50, kind=kind, link="L"))
        ops.move_clips(timeline, [timeline.video_tracks[0].clips[0]], 200)
        assert timeline.video_tracks[0].clips[0].tl_start == 200
        assert timeline.audio_tracks[0].clips[0].tl_start == 200, "audio followed video"

    def test_move_onto_occupied_space_is_refused(self, linked):
        with pytest.raises(TimelineError, match="in the way"):
            ops.move_clips(linked, [linked.video_tracks[0].clips[0]], 100)

    def test_failed_move_leaves_the_timeline_untouched(self, linked):
        before = [(c.tl_start, c.tl_end) for c in linked.video_tracks[0].clips]
        with pytest.raises(TimelineError):
            ops.move_clips(linked, [linked.video_tracks[0].clips[0]], 100)
        assert [(c.tl_start, c.tl_end) for c in linked.video_tracks[0].clips] == before
    ...
    def test_move_between_tracks(self, timeline):
        clip = make_clip(0, 50)
        timeline.video_tracks[0].insert(clip)
        ops.move_clips(timeline, [clip], 0, target_track=timeline.video_tracks[1])
...
class TestTrim:
    ...
    def test_trim_applies_to_link_group(self, timeline):
        for kind in ("video", "audio"):
            timeline.lane_for(kind).insert(make_clip(0, 100, src_length=200, kind=kind, link="L"))
        ops.trim(timeline, timeline.video_tracks[0].clips[0], "out", 60)
        assert timeline.audio_tracks[0].clips[0].duration == 60, "audio trimmed to match"
    ...
    def test_trim_group_uses_most_restrictive_limit(self, timeline):
        ...
        timeline.video_tracks[0].insert(make_clip(0, 100, src_length=200, kind="video", link="L"))
        timeline.audio_tracks[0].insert(make_clip(0, 100, src_length=120, kind="audio", link="L"))
        assert ops.trim(timeline, timeline.video_tracks[0].clips[0], "out", 200) == 120
...
class TestLinking:
    def test_unlink_then_move_independently(self, linked):
        ...
        assert linked.video_tracks[0].clips[-1].tl_start == 300
        assert linked.audio_tracks[0].clips[0].tl_start == 0, "audio stayed put"
...
class TestUndo:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("lift", lambda t: ops.lift(t, [t.video_tracks[0].clips[0]])),
            ("ripple", lambda t: ops.ripple_delete(t, [t.video_tracks[0].clips[0]])),
            ("move", lambda t: ops.move_clips(t, [t.video_tracks[0].clips[1]], 300)),
            ("trim out", lambda t: ops.trim(t, t.video_tracks[0].clips[0], "out", 60)),
            ("trim in", lambda t: ops.trim(t, t.video_tracks[0].clips[0], "in", 30)),
            ("range", lambda t: ops.ripple_delete_range(t, 25, 175)),
            ("unlink", lambda t: ops.unlink(t, t.video_tracks[0].clips[0])),
        ],
    )
    ...
    def test_a_rejected_edit_is_not_recorded(self, linked):
        ...
        before = self._state(linked)
        with pytest.raises(TimelineError):
            stack.apply("bad move", lambda t: ops.move_clips(t, [t.video_tracks[0].clips[0]], 100))
        assert self._state(linked) == before
    ...
    def test_nested_apply_collapses_to_one_step(self, linked):
        ...
        assert len(linked.video_tracks[0].clips) == 4
        stack.undo()
        assert len(linked.video_tracks[0].clips) == 2, "one undo reverted both cuts"
